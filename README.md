...
本分支是基于 [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api) 的魔改版本，主要进行了以下定制化修改：

### 界面风格定制

| 修改项 | 原版 | 魔改版 |
|--------|------|--------|
| 主题配色 | Teal 青色 (#14b8a6) | Cyan 青色 (#06b6d4) |
| 默认背景 | 浅色模式 | 全黑极客风格 |
| 首页样式 | 浅灰渐变背景 | 深色赛博朋克风格 |
| Logo | 默认灰色 | 青色高亮 |
...
- 去除多余引导元素，更简洁
- 去除 GitHub 链接推广
- **强制深色模式**：取消主题切换开关，始终使用深色主题

### 功能调整

- **删除 SaaS 相关功能**：移除在线体验、官方推广链接、演示账号等
- **移除部分测试功能**：简化账号测试流程
- **删除计费模块相关代码**：适合个人/私有部署
- **删除价格配置相关代码**：不再依赖价格表
...
```bash
curl -sSL https://raw.githubusercontent.com/TangSengVIP/sub2api/dev/deploy/install.sh | sudo bash
```
...
# 卸载
curl -sSL https://raw.githubusercontent.com/TangSengVIP/sub2api/dev/deploy/install.sh | sudo bash -s -- uninstall -y
```
...
| **docker-compose.yml** | 命名卷 | 需要 docker 命令 | 简单设置 |
